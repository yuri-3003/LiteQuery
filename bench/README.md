...
|---|---|---|---|
| `COUNT(*)` | ~12 M rows/s | ~306 M rows/s | ~26x |
| `SUM(x)` | ~14 M rows/s | ~241 M rows/s | ~17x |
| `AVG(x) WHERE x>500` | ~8 M rows/s | ~90 M rows/s | ~10x |
| `GROUP BY cat SUM(x)` | ~7 M rows/s | ~23 M rows/s | ~3x |
| filtered `GROUP BY` | ~5 M rows/s | ~27 M rows/s | ~5x |
...
|---|---|---|---|---|
| load | 0.28 s | 0.92 s | 3.2x | — |
| `COUNT(*)` | 0.017 s | 0.001 s | **0.1x** | ✓ match |
| `SUM(x)` | 0.021 s | 0.118 s | **5.6x** | ✓ match |
| `AVG(x) WHERE x>500` | 0.054 s | 0.181 s | **3.3x** | ✓ match |
| `GROUP BY cat SUM(x)` | 0.220 s | 1.395 s | **6.3x** | ✓ match |
| filtered `GROUP BY` | 0.200 s | 1.293 s | **6.5x** | ✓ match |

LiteQuery is 3-7x faster on the analytical aggregations, scanning the whole
table columnarly. SQLite wins `COUNT(*)`, which it answers from table metadata
rather than scanning (LiteQuery does not special-case this yet). Every LiteQuery
result matches SQLite to within floating-point tolerance.
